...
### 物理テナンシー
* Dedicated Host
  - 完全にユーザー専用として利用できる物理サーバ
  - 同じAWSアカウントに属するIAMユーザー/グループに対しても、
    権限付与によりそれらと物理サーバを共有しないことが可能
* ハードウェア専有インスタンス
  - 専用HWのVPCで実行されるEC2インスタンス
  - 他のAWSアカウントに属するインスタンスとは物理的に分離
  - Dedicated Hostと違い同じAWSアカウントとHWを共有する可能性がある
* Bare Metal
  - アプリケーションがサーバのプロセッサやメモリ、
    OSの直下層のHWに直接アクセス可能
...
| Connection Draining | リソースが登録解除or異常が発生した場合に、そのリソースへの新規リクエスト送信を中止 |
