...
2. リザーブドインスタンス
    ...
    * スタンダートとコンバーティブルの購入タイプが存在（後述）
    * 途中で不要になった場合は
      「リザーブドインスタンスマーケットプレイス」で販売することも可能（後述）
3. スポットインスタンス
...
* コンバーティブル
    + スタンダードと比べて割引率は低い
    + 期間中のインスタンスタイプ等の変更は可能

| | スタンダード | コンバーティブル |
| --- | --- | --- |
| 利用期間 | 1年（40%割引）<br>3年（60%割引） | 1年（31%割引）<br>3年（54%割引） |
| AZ/インスタンスサイズ<br>/ネットワークタイプ変更可否 | ○ | ○ |
| インスタンスファミリー<br>/OS/テナンシー<br>支払いオプション可否 | × | ○ |
| リザーブドインスタンス<br>マーケットプレイスでの販売 | ○ | (今後可となる予定) |

また、リザーブド購入オプションはEC2インスタンス以外にも存在する。
...
### EC2 Image Builder
* AMIイメージの構築・テスト・デプロイを簡素化するための展開用のツール
* スケジュール起動も可能なので定期的なAMIの更新が可能
...
### EC2フリート
![](https://docs.aws.amazon.com/ja_jp/AWSEC2/latest/UserGuide/images/ec2-fleet.png)

オンデマンドインスタンスとスポットインスタンスで構成されるインスタンスグループを定義する仕組み
...
* インスタンス間の通信速度を高速化させる機能（ネットワークのレイテンシーを削減する）
* **単一AZ内**の複数のEC2インスタンスをグループ化することで、物理的になるべく近いインスタンスとして起動
...
* WebSocketとHTTP/2のリクエスト受付にも対応
* 1ターゲットに複数ポートを登録可能
* ターゲットグループに対応
  - 複数のターゲットをグループ化したもの
    - ターゲットグループのターゲットはAZを跨ぐことができる
    - ターゲットグループに対するヘルスチェックが可能
* 加重ロードバランシングが可能
...
### コンテナのトラフィック分散
![](https://camo.qiitausercontent.com/1d700ee2b42ff6fc784fb48f454a03601f5a366f/68747470733a2f2f71696974612d696d6167652d73746f72652e73332e616d617a6f6e6177732e636f6d2f302f3134333533342f39613962353938652d616534352d643864642d653064642d3961363834346633383533332e706e67)

* ECSにトラフィックを分散させる仕組みがないため、ELBと組み合わせる必要がある
* https://qiita.com/IgnorantCoder/items/d4f16b1aadd1c03c0e26 の作成例が詳しい
...
* DockerでLambda関数を作成すれば、他の言語も使用可能
